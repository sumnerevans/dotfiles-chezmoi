...
if [[ $DP1 != "" ]]; then
    ...
    xrandr --output DP1 --mode 1920x1200 --right-of eDP1 --rotate left
elif [[ $DP2 != "" ]]; then
    # External monitor at home (other USB-C port)
    xrandr --output DP2 --mode 1920x1200 --right-of eDP1 --rotate left
elif [[ $DP4 != "" ]]; then
    # Double monitor set up with Desktop
    xrandr --output DP-0 --mode 2560x1440 --rate 144.00
    xrandr --output DP-4 --mode 1920x1200 --right-of DP-0 --rotate left
elif [[ $DP23 != "" ]]; then
    # External monitor at home (USB-C dock)
    xrandr --output DP2-3 --mode 1920x1200 --right-of eDP1 --rotate left
elif [[ $DP13 != "" ]]; then
    # External monitor at home (USB-C dock)
    xrandr --output DP1-3 --mode 1920x1200 --right-of eDP1 --rotate left
elif [[ $DP3 != "" ]]; then
    # External monitor at home, from MacBook Pro (make sure to scale).
    xrandr --output DP-3 --mode 1920x1200 --right-of DP-2 --scale 1.5x1.5 --rotate left
elif [[ $HDMI2 != "" ]]; then
    # External monitor at home, from MacBook Pro (make sure to scale).
